...
##Select the build name
export BUILD_NAME=15.01-RC5-Makerbot
...
##Do we download and build the Ultimaker firmwares ?
ULTIMAKER_FIRMWARE=0

##Do we need to create the final archives ?
ARCHIVE_FOR_DISTRIBUTION=0

##Do we want to build the CuraEngine ?
BUILD_CURA_ENGINE=1

##Do we build CuraEngine from GIT or local ressource
##Comment line to build from github repo
export CURA_ENGINE_REPO=../CuraEngine-15.01-RC5-Makerbot
...
if (( ${ULTIMAKER_FIRMWARE} )); then
	#Build the Ultimaker Original firmwares.
	printf '\e[1;46mBUILDING ULTIMAKER FIRMWARES\e[0m\n'
...
if [ "$BUILD_TARGET" = "darwin" ]; then
	...
	rm -rf Cura-${BUILD_NAME}.app

	printf '\e[1;46mBUILDING CURA PYTHON APP\e[0m\n'
	python build_app.py py2app
	rc=$?
	if [[ $rc != 0 ]]; then
		printf '\e[1;41mERROR : CANNOT BUILD APP\e[0m\n'
		exit 1
	fi
	...
	if (( ${BUILD_CURA_ENGINE} )); then
		printf '\e[1;46mBUILDING CURA ENGINE\e[0m\n'
		rm -rf CuraEngine
	...
	printf '\e[1;46mCOPYING QUICKLOOK PLUGIN\e[0m\n'
	...
	cp -a dist/Cura.app ../../Cura-${BUILD_NAME}.app
	printf '\e[1;44mBUILD IS DONE !\e[0m\n'
	exit
fi
